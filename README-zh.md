...
本项目是由 [Websoft9](https://www.websoft9.com) 研发的 **云原生程序** ，大大简化 Jetty 复杂的安装及配置。

## 系统要求

安装本项目，确保符合如下的条件（[详细参考](https://github.com/jetty/docker#recommended-system-requirements)）：
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r jetty
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r jetty -p
```
2.复制package包后安装

将压缩的package包复制到想要安装的服务器或虚拟机，登录 Linux，运行下面的**自动化命令**即可启动安装并显示安装结果。
```
sudo bash install-jetty
```
...
```
git clone --depth=1 https://github.com/Websoft9/docker-jetty
cd docker-jetty
echo aaa  
echo bbb  
...
| 名称 | 端口号 | 用途 |  必要性 |
| --- | --- | --- | --- |
| jetty-server | 9002 | 浏览器访问 Jetty | Y |
| phpmyadmin | 9090 | 数据库可视化管理工具 | Y |
## 文档

[Jetty 管理员手册](https://support.websoft9.com/docs/jetty)

## 企业级支持

如果需要企业级支持，请订阅我们提供的 [Jetty 企业级支持版](https://apps.websoft9.com/jetty) 
